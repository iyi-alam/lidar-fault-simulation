...
class SnowSim:
    ...
    def simulate(self, pc: np.ndarray, alpha: float = 0.0, r_max: float = 100.0, reflectivity: float = 0.9):
        pc = deepcopy(pc)
...
def init_worker(dataroot, dataversion, snowfall_rate, terminal_velocity, 
                mode, save_root, alpha, r_max, reflectivity):
    global GLOBAL_NUSC
    global GLOBAL_SNOW_SIM
    global GLOBAL_ARGS
    ...
    GLOBAL_SNOW_SIM = SnowSim(snowfall_rate, terminal_velocity, mode)
...
def transfer_snow_particles(sample_snow_pts, sample_token, sweep_token, jitter_std=0.1):
    ...
    snow_global_xyz = transform_xyz(sample_snow_pts[:, :3], T_sample)
    snow_sweep_xyz = transform_xyz(snow_global_xyz, T_global_to_sweep)
...
def get_sweep_chain(start_token, max_sweeps=None):
    sweeps = []
    token = start_token
    count = 0
...
def build_unique_groups(nusc: NuScenes, samples_list, max_sweeps=None, max_samples=None):
    ...
    for sample_token in samples_token_list:
        ...
        for sd in sweep_chain:
            if sd["token"] in visited_tokens:
                continue
        ...
        groups.append({
            ...
            "owned_chain": [sd["token"] for sd in chain],
        })
...
def process_group(group):
    ...
    key_sim = GLOBAL_SNOW_SIM.simulate(key_orig.copy(), alpha=GLOBAL_ARGS["alpha"], r_max=GLOBAL_ARGS["r_max"], reflectivity=GLOBAL_ARGS["reflectivity"])

    key_out = os.path.join(save_root, key_rel)
    save_pc(key_out, key_sim)
    ...
    for token in owned_tokens:
        ...
        transferred_snow = transfer_snow_particles(
            sample_snow_pts,
            key_token,
            token,
        )
        ...
        out_path = os.path.join(save_root, sweep_rel)
        save_pc(out_path, final_sweep)
...
def parse_args():
    ...
    parser.add_argument("--alpha", default=0.06, type=float)
    parser.add_argument("--r_max", default=100.0, type=float)
    parser.add_argument("--reflectivity", default=0.9, type=float)
    parser.add_argument("--max_samples", default=None, type=int)
    parser.add_argument("--max_sweeps", default=None, type=int)
...
if __name__ == "__main__":
    ...
    print("Snow simulation complete.")
