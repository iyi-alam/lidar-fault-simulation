...
PI = np.pi
#SAVE_DIR = str(Path.home() / 'fault_sim' / 'LiDAR_snow_sim' / 'snowflakes')
SAVE_DIR = os.path.dirname(os.path.abspath(__file__))
SAVE_DIR = os.path.join(SAVE_DIR, '../../snowflakes')
...
def dart_throwing(occupancy_ratio: float,
    ...
    while area_occupied < area_occupied_global:
        ...
        percentage = np.round((area_occupied * 100 / area_occupied_global), 4)
        #print(f"P_dia: {particle_diameter} | % Area: {percentage} %") #, end='\r', flush=True)
...
if __name__ == '__main__':
    ...
    # r_s_s = np.linspace(0.5, 2.5, 20)                                            # mm/h
    # v_s_s = np.linspace(0.2, 2, 10)                                             # m/s
    r_s_s = np.array([20.0, 30.0, 40.0, 50.0])
    v_s_s = np.array([2.0])

    snow_rate = np.linspace(0.5, 2.5, 5)
    rain_rate = [snowfall_rate_to_rainfall_rate(snowfall_rate=x, terminal_velocity=2.0, snowflake_diameter=0.003) for x in snow_rate]
    print(rain_rate)
    ...
    # # print(runs)
    # # dummy = itertools.product(m, runs, n)
    # # print(list(dummy))
    ...
    # #process_map(do_in_parallel, paramlist, max_workers= 1, chunksize=1)
