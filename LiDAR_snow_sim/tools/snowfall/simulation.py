...
from OpenPCDet.pcdet.utils import calibration_kitti #type:ignore
...
def get_occlusions(beam_angles: np.ndarray, ranges_orig: np.ndarray, root_path: str, particle_file: str,
    ...
    if root_path:
        ...
        path = Path(__file__).parent.parent.parent.absolute() / 'snowflakes' / particle_file
...
def augment(pc: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
            ...
            root_path: str=None, flat_earth = False) -> np.ndarray:
    """
    ...
    # else:
    #     w = [0, 0, 1]
    #     h = -1.55                           # Use default flat earth assumption
    ...
    partial_func = functools.partial(process_single_channel, root_path, particle_file_prefix,
    ...
    # ----------------------------------------------------------------------------------------#
    ## DISABLED PARALLEL PROCESSING BY CHANNELS AND ENABLED PARALLEL PROCESSING USING FILES
    # ----------------------------------------------------------------------------------------#
...
if __name__ == '__main__':
    ...
    # print(
    #     dict(
    #     beam_angles=beam_limit_angles,
    #     ranges_orig=ranges,
    #     root_path=None,
    #     particle_file=test_particles,
    #     beam_divergence=float(np.degrees(angular_resolution))
    #     )
    # )
    result = get_occlusions(
        beam_angles=beam_limit_angles,
        ranges_orig=ranges,
        root_path=None,
        particle_file= "gunn_2.621627143512277_7.716049382716048e-07_1.npy",
        beam_divergence=float(np.degrees(angular_resolution))
    )
