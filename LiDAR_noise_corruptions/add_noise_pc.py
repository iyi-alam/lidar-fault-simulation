...
def background_noise(pointcloud, severity):
    N, C = pointcloud.shape
    c = [N//45, N//40, N//35, N//30, N//10][severity-1]
...
def upsampling(pointcloud, severity):
    N, C = pointcloud.shape
    c = [N//100, N//80, N//60, N//40, N//10][severity-1]
    # Fast random choice
    index = np.random.randint(0, N, size=c)  # much faster than choice(..., replace=False)
    add = pointcloud[index] + np.random.uniform(-1.0, 1.0, (c, C)).astype(np.float32)

    # Add a label to original point

    return np.concatenate((pointcloud, add), axis=0)
...
def gaussian_noise_radial(pointcloud, severity):
    N, C = pointcloud.shape
    c = [0.04, 0.06, 0.08, 0.10, 0.20][severity-1]
