...
"""
# Append LISA path to system path
import sys
import os
...
class Lisa:
    ...
    def __init__(self, rain_rate = 30, snow_rate = 0.5, m=1.328, lam=905, rmax=200, rmin=1.5, bdiv=3e-3, dst=0.05,
        ...
        if atm_model == 'rain':
            self.rate = rain_rate
            self.N_model = self.N_model_rain
            self.N_tot   = self.N_tot_rain
            self.N_sam   = self.N_sam_rain

        elif atm_model == 'snow':
            self.rate = snow_rate
            self.m = 1.3031  # refractive index of ice
            self.N_model = self.N_model_snow
            self.N_tot   = self.N_tot_snow
            self.N_sam   = self.N_sam_snow

        else:
            raise ValueError(f"Unsupported atm_model: {atm_model}")
        
        # if not disable_multi_lisa:
    ...
    def simulate(self, pc: np.ndarray):
        return self.augment_mc(pc, self.rate)
    
    # ============================================================
    #               DISTRIBUTION FUNCTIONS (RAIN)
    # ============================================================

    def N_model_rain(self, D, Rr):
        return n_mp_rain(D, Rr)

    def N_tot_rain(self, Rr, dst):
        return n_mp_tot_rain(Rr, dst)

    def N_sam_rain(self, Rr, N, dst):
        return mp_sample_rain(Rr, N, dst)

    # ============================================================
    #               DISTRIBUTION FUNCTIONS (SNOW)
    # ============================================================

    def N_model_snow(self, D, Rr):
        return n_mg_snow(D, Rr)

    def N_tot_snow(self, Rr, dst):
        return n_mg_tot_snow(Rr, dst)

    def N_sam_snow(self, Rr, N, dst):
        return mg_sample_snow(Rr, N, dst)
    ...
    def haze_point_cloud(self,pts_3D,Rr=0):
        ...
        elif (self.atm_model == 'chu_hogg_fog') or (self.atm_model=='strong_advection_fog') or (self.atm_model=='moderate_advection_fog'): #type:ignore
        	Nd  = self.N_model(self.D) # density of water droplets (m^-3)
        else: #type: ignore
        	print('Warning: weather model not implemented')
        alpha, _ = alpha_beta(Nd, self.D, self.q_ext, self.q_back)     # extinction coeff. (1/m) #type:ignore
        #######################################################################
