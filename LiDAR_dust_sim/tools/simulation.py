...
def process_single_channel(root_path: str, orig_pc: np.ndarray, beam_divergence: float,
                           order: List[int], channel: int) -> Tuple:
    """
    ...
    min_intensity = 0  # not all channels contain this info
    max_intensity = 255

    particle_file = f'particles_{index}.npy'
    ...
    #print(f"Time Taken for this channel: {end - start:.6f} seconds")
...
def get_occlusions(beam_angles: np.ndarray, ranges_orig: np.ndarray, root_path: str, particle_file: str,
    ...
    if root_path:
        path = Path(root_path) / 'training' / 'particles' / 'npy' / particle_file
    else:
        path = Path(__file__).parent.parent.absolute() / 'particles' / particle_file
...
def augment(pc: np.ndarray, beam_divergence: float, shuffle: bool = True,
            show_progressbar: bool=False, root_path: str=None, num_channels: int = 32) -> Tuple:
    """
    ...
    sensor_info = Path(__file__).parent.parent.resolve() / 'calib' / '20171102_64E_S3.yaml'
    print(sensor_info)
    ...
    partial_func = functools.partial(process_single_channel, root_path,
                                                        orig_pc, beam_divergence, order)

    for i in channels:
        channel_list[i] = partial_func(channel = i)
    ...
    #                                                  beam_divergence, order), channels)
...
def xsi(R: float, R_1: float = 0.9, R_2: float = 1.0) -> float:
    return np.clip((R - R_1)/(R_2 - R_1), 0, 1)
